...
1. Scans all EBS volumes
2. Detects volumes in `available` state
3. Verifies retention conditions
4. Sends notification alerts
5. Deletes unused volumes automatically
...
1. Retrieves all running EC2 instances
2. Filters only instances tagged for automation
3. Analyzes CloudWatch CPU metrics
4. Detects prolonged idle usage
5. Sends SNS notifications
6. Stops idle instances automatically
...
1. Monitors EC2 utilization metrics
2. Identifies underutilized compute resources
3. Evaluates current instance types
4. Generates optimized instance recommendations
5. Sends optimization reports via SNS
...
Built a serverless AWS cloud optimization platform using Lambda, EventBridge, CloudWatch, SNS, and EC2 APIs to automatically identify unused EBS volumes, stop idle EC2 instances, and generate infrastructure rightsizing recommendations, enabling intelligent cost optimization and cloud governance through event-driven automation.

---

# Conclusion

This project demonstrates practical implementation of cloud-native automation and FinOps engineering using AWS services. By combining serverless computing, monitoring, event-driven workflows, and intelligent optimization logic, the platform provides an automated solution for reducing unnecessary cloud expenditure while improving operational efficiency and governance.
